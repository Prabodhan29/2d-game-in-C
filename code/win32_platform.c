...
static String os_read_save_file() {
    return os_read_entire_file("C:/Users/prabo/Desktop/block_shooter_game/data/save.brk");
}

static b32 os_write_save_file(String data) {
    b32 result = false;
    
    HANDLE file_handle = CreateFileA("C:/Users/prabo/Desktop/block_shooter_game/data/save.brk", GENERIC_WRITE, 0, 0, CREATE_ALWAYS, 0, 0);
    if (file_handle == INVALID_HANDLE_VALUE) {
        assert(0);
        return result;
    }
...
static b32 win32_do_next_queue_job(Os_Job_Queue *queue) {
...
static void win32_create_queue(Os_Job_Queue *queue, int number_of_thread) {
    zero_struct(*queue);
...
static void os_add_job_to_queue(Os_Job_Queue *queue, Os_Job_Callback *callback, void *data) {
...
static void win32_init_audio(HWND window) {
...
static void win32_clear_sound_buffer() {
    void *region_1;
    DWORD region_1_size;
    void *region_2;
    DWORD region_2_size;
...
static void win32_fill_sound_buffer(DWORD byte_to_lock, DWORD bytes_to_write) {
    void *region_1;
    DWORD region_1_size;
    void *region_2;
    DWORD region_2_size;
...
int CALLBACK WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nShowCmd) {
    ...
    while(running) {
        ...
        for (int i = 0; i < BUTTON_COUNT; i++) {
            win32_input.buttons[i].changed = false;
        }
        ...
        // Simulation
        if (!win32_paused) 
            update_game(&win32_input, last_dt);

        end_profiling(PROFILING_GAME);
        
        if (!win32_paused) 
            render_profiler(last_dt);
