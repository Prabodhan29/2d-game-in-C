...
static void draw_rect_in_pixels_transparent(int x0, int y0, int x1, int y1, u32 color, f32 alpha) {
...
inline f32 calculate_aspect_multipler() {
...
static v2 pixels_dp_to_world(v2i pixels_coord) {
...
static v2 pixels_to_world(v2i pixels_coord) {
    f32 aspect_multiplier = calculate_aspect_multipler();
...
static void draw_transparent_rect(v2 p, v2 half_size, u32 color, f32 alpha) {
...
static void draw_arena_rects(v2 p, f32 left_most, f32 right_most, f32 half_size_y, u32 color) {
    begin_profiling(PROFILING_DRAW_BACKGROUND);
...
static void clear_arena_screen(v2 p, f32 left_most, f32 right_most, f32 half_size_y, u32 color) {
    begin_profiling(PROFILING_DRAW_BACKGROUND);
...
static f32 draw_number(int number, v2 p, f32 size, u32 color, int number_of_leading_zeros) {
    ...
    if(draw_minus) 
        draw_rect((v2){p.x, p.y}, (v2){2.f*half_square_size, half_square_size}, color);
    
    return p.x;
}

static void draw_f32(f32 num, v2 p, f32 size, b32 color) {
    int whole = (int)num;
...
static void draw_transparent_rotated_rect(v2 p, v2 half_size, f32 angle, u32 color, f32 alpha) { 
    ...
    v2 position_v = (v2){(f32)render_buffer.width * .5f, (f32)render_buffer.height * .5f};
...
static void draw_rotated_rect(v2 p, v2 half_size, f32 angle, u32 color) { //In degrees
    begin_profiling(PROFILING_DRAW_ROTATED);
...
static Bitmap load_png(char *path) {
    Bitmap result;
...
static void draw_bitmap(Bitmap *bitmap, v2 p, v2 half_size, f32 alpha_multiplier) {
    ...
    for (int y = y0; y < y1; y++) {
        ...
        u32 *source_pixels = bitmap->pixels + stride;
...
static void draw_rect_subpixel(v2 p, v2 half_size, u32 color) {
    f32 aspect_multiplier = calculate_aspect_multipler();
    p = sub_v2(p, cam_p);
...
inline int get_letter_index(char c) {
    if (c == '.') return 'Z'-'A'+1;
    if (c == '/') return 'Z'-'A'+2;
    return c-'A';
}
...
static f32 get_word_align_offset(int text_align, char *word, f32 size) {
    // assert(*word);
...
static void draw_text(char *text, v2 p, f32 size, u32 *colors, int color_count, int first_color, int text_align) {
    ...
    f32 block_offset_x = size*1.6f*2.f + size*.8f;
