...
static void set_volume(Playing_Sound *sound, f32 volume) {
    volume = max(0.f, volume);
    sound->target_volume = volume;
    sound->volume = volume;
}

static Playing_Sound* play_sound(Loaded_Sound *sound, b32 looping) {
...
static Playing_Sound* play_sound_with_variation(Loaded_Sound *sound, f32 variation, f32 *source) {
...
static void stop_sound(Playing_Sound *sound) {
    sound->active = false;
    sound->next_free = first_free_sound;
    first_free_sound = sound;
}

static void update_game_audio(Game_Sound_Buffer *sound_buffer, f32 dt) {
