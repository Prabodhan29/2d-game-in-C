...
char *level_names[] = {"BREAK\\OUT", "SUPER\\BREAKOUT", "\\PONG", "\\TETRIS","SPACE\\INVADERS"};

f32 test;

static void update_menu(Input *input, f32 dt) {
        ...
        if (input->mouse_dp.x != 0 && input->mouse_dp.y != 0) {
            ...
            if (!save_data.levels[new_hot_level].locked) {
                ...
                if (old_hot != hot_level) {
                    play_sound(&interface_sound, false);
                }
            } 
            else
                hot_level = L_COUNT;
    ...
    if (use_light_logo) {
        u32 color = 0xffffff;
        draw_bitmap(&bitmap_half_logo_light, (v2){0, 28}, (v2){28.57f, 2.10f}, 1.f);
    ...
    {
        u32 color = 0xfffca2;
        draw_text("CREATED BY PRABODHAN", (v2){0, 13}, .2f, &color, 1, 0, TEXT_ALIGN_CENTER); // Use uppercase words only.
        color = 0xffda80;
        draw_text("", (v2){0, 8}, .175f, &color, 1, 0, TEXT_ALIGN_CENTER);
    }
}
...
global_varible Level_Transition level_transition;

static void draw_level_transtion(f32 t, f32 dt) {
    //? Menu Drawing
    ...
    if (t < 0.5f) {
        if (!level_transition.played_spring_sound_effect) {
            ...
            Playing_Sound *sound = play_sound(&spring_sound, false);
...
static void draw_level_transtion_to_menu(f32 t, f32 dt) {
