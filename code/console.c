...
static void print_int(int number) {
    ...
    if (current_message >= array_count(messages)) 
        current_message = 0;
...
static void print_f32(f32 number) {
    Message *message = messages + current_message++;
    if (current_message >= array_count(messages)) 
        current_message = 0;
...
static void print_v2i(v2i number) {
    Message *message = messages + current_message++;
    if (current_message >= array_count(messages)) 
        current_message = 0;
...
static void print_v2(v2 number) {
    Message *message = messages + current_message++;
    if (current_message >= array_count(messages)) 
        current_message = 0;
...
static void draw_messages(f32 dt) {
    f32 original_x = -76;
    v2 p = {original_x, 40};
