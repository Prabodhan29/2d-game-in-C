...
Level_Info level_info[] = {
       {0x551100, 0x220500}, // L01_NORMAL
       {0x804001, 0xff7d03}, // L02_WALL
       {0x008080, 0x00ffff}, // L05_PONG
       {0x406949, 0x89da59}, // L03_TETRIS  
       {0x333333, 0x111111}, // L06_INVADERS
};
...
inline int random_powerup() {
    return random_int_in_range(1, POWERUP_LAST);
}

inline int random_powerdown() {
...
inline void add_screenshake(f32 factor) {
    cam_dp = (v2){random_f32_in_range(-factor, factor), random_f32_in_range(-factor, factor)};
}

inline Particle* spawn_particle(v2 p, f32 dp_scale, v2 half_size, f32 angle, f32 life, f32 life_d, u32 color) {
    Particle *particle = particles + next_particle++;
    if (next_particle >= array_count(particles)) 
        next_particle = 0;
...
inline void spawn_particle_explosion(int count, v2 p, f32 dp_scale, f32 base_size, f32 base_life, u32 color) {
...
inline void increase_ball_size(Ball *ball) {
    ball->half_size += .2f/ball->half_size;
}

static void reset_and_reverse_ball_dp_y(Ball *ball) {
    if (ball->dp.y > 0) 
...
static void do_wall_hit_effects(v2 p, f32 *sound_source) {
...
static void process_ball_when_dp_y_down(Ball *ball) {
    if (ball->flags & BALL_DESTROYED_ON_DP_Y_DOWN) {
        if (ball->sound) 
            set_volume(ball->sound, 0.f);

        if (ball->comet_sound) 
            set_volume(ball->comet_sound, 0.f);
...
static Block* get_next_available_block() {
...
static Ball* get_next_available_ball_and_zero() {
...
static void spawn_triple_shot_balls() {
    Ball *ball;
    for (int i = 0; i < 2; i++) {
...
static void reset_power_blocks() {
...
static void spawn_power_block(Power_Block_Kind kind, v2 p) {
    Power_Block *power_block = power_blocks + next_power_block++;
    if (next_power_block >= array_count(power_blocks)) 
        next_power_block = 0;
...
static b32 maybe_update_highscore() {
    b32 result = false;
...
static void spawn_tetris_shape(int random_lane, f32 size, b32 neighbours, Block *parent_block) {
    ...
    if(choice == 0)    
        color = make_color(random_color, 255-random_color, 255-random_color);

    else if(choice == 2) 
        color = make_color(255-random_color, random_color,255-random_color);

    else if(choice == 3) 
        color = make_color(255-random_color, 255-random_color, random_color);
    
    else if(choice == 4) 
        color = make_color(255-random_color, (u8)random_int_in_range(200, 255), random_color);

    else 
        color = make_color(255-random_color, random_color, (u8)random_int_in_range(200, 255));
        ...
        while(*at) {
            if (*at++ != ' ') {
                ...
                if (tetris->shapes_spawned == 1 || tetris->level_progression == 3)
                    ...
                    if (random_choice(3)) 
                        block->power_block = random_int_in_range(0, POWERUP_LAST);
                }
            }
            p.x += offset;
        }
        p.y -= offset;
        if (i != TETRIS_SHAPE_HEIGHT-1) 
            p.x = original_x;
    }
...
static void test_for_win_condition() {
    blocks_destroyed++;
    score += player_life + touchless_bonus++;
    
    if(level_score_changed()) 
        return;
    ...
    if(blocks_destroyed == num_blocks) {
        ...
        if(current_level < L_COUNT-1) {
            if(save_data.levels[current_level+1].locked) {
...
static void block_destroyed(Block *block, Ball* ball, b32 maybe_destroy_neighbours) {
    block->life = 0;
    ...
    if (maybe_destroy_neighbours) {
        ...
        if (current_time - last_hit_sound_played < .5f) {
            ...
            next_hit_sound_to_play = 0;

        play_sound(&hit_sounds[next_hit_sound_to_play], false);
...
inline b32 do_ball_block_collision(Ball *ball, Block *block, f32 *min_t, Block **hit_block, b32 *ball_update_axis) {
    assert(block->life > 0);
    ...
    if (diff != 0) {
        f32 collision_point;
        if (ball->dp.y > 0) 
            collision_point = block->p.y - block->half_size.y - ball->half_size;
    ...
    if (diff != 0) {
        f32 collision_point;
        if (ball->dp.x > 0) 
            collision_point = block->p.x - block->half_size.x - ball->half_size;
...
static void create_invader(v2 p) { 
    char *invader[] = {
        "  0     0",
        "   0   0",
        "  0000000",
...
static void calculate_all_neighbours() {
...
static void create_block_block(int num_x, int num_y, v2 relative_spacing, f32 x_offset, f32 y_offset, v2 block_half_size, f32 base_speed_multiplier, v2 *parent_p) {
...
static void start_game(Level level) {
    {
...
static void lose_life(b32 instakill) {
    add_screenshake(200);
...
static void post_simulate_level(Level level, f32 dt) {
    switch(level) {
        case L03_TETRIS: {
            Level_Tetris_State *tetris = &level_state.tetris;
...
static void simulate_level(Level level, f32 dt) {
    switch(level) {
        case L06_INVADERS: {
        ...
        case L05_PONG: {
            ...
            if (absf(balls[0].p.x - pong->enemy_p.x) > 6.f) {
                if (balls[0].p.x > pong->enemy_p.x) 
                    ddp.x += 500.f;

                else if (balls[0].p.x < pong->enemy_p.x)
                    ddp.x += -500.f;
            }
            
            v2 desired_dp = add_v2(pong->enemy_dp, mul_v2(ddp, dt));
            v2 desired_p = add_v2(add_v2(pong->enemy_p, mul_v2(desired_dp, dt)), mul_v2(ddp, square(dt)));
            ...
            if (desired_p.x > arena_right_wall_visual_p - pong->enemy_half_size.x) {
                ...
                if (!pong->is_collising_with_wall) {
                    add_screenshake(150);
                    Playing_Sound *sound = play_sound_with_variation(&player_wall_sound, .15f, &pong->enemy_p.x);
        ...
        case L03_TETRIS: {
            ...
            for (int i = 0; i < level_state.tetris.shapes_spawned; i++) {
                tetris->enemies_p[i].y -= (TETRIS_SHAPE_INITIAL_Y*2+tetris->enemies_p[i].y) * .035f*dt;
            }
...
static void simulate_block_for_level(Block *block, Level level, f32 dt) {
    switch(level) {
        case L06_INVADERS: {
            if (block->parent_p) 
