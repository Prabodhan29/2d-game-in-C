...
inline Riff_Iterator parse_chunk_at(void *at, void* stop) {
    Riff_Iterator iter;
...
inline b32 is_riff_iterator_valid(Riff_Iterator iter) {
    return iter.at < iter.stop;
}

inline Riff_Iterator next_chunk(Riff_Iterator iter) {
...
inline void * get_chunk_data(Riff_Iterator iter) {
    return iter.at + sizeof(Wav_Chunk);
}

inline u32 get_type(Riff_Iterator iter) {
    Wav_Chunk *chunk = (Wav_Chunk*)iter.at;
    return chunk->id;
}

inline u32 get_chunk_data_size(Riff_Iterator iter) {
    Wav_Chunk *chunk = (Wav_Chunk*)iter.at;
    return chunk->size;
}

static Loaded_Sound load_wav_from_memory(String s) {
...
static Loaded_Sound load_wav(char *file_name) {
